...
vector<vector<string>> find_cod (string id, vector<tuple<string,int,int>> vtup, unordered_map<string, string> &umCod)
{
	...
	for (auto t: vtup)
		if (get<0>(t).size() > 9){
			...
			string c1 = rev.substr(0,3);
			reverse (c1.begin(), c1.end());
			string c2 = rev.substr(3,3);
			reverse (c2.begin(), c2.end());
			string c3 = rev.substr(6,3);
...
cout <<"Analizing inter structures Structure . . ." <<flush;	
...
cout <<"Size of structures analysed: " <<vmat.size() <<endl;
	...
	freq_codons ("Coil_by_first_second_third_codons_chisq_last_condons.tsv", vmat);
